...
class Measurement:
    ...
    @staticmethod
    def to_message(meas):
        return crowd_navigation_msgs.msg.Measurement(meas.x, meas.y, meas.id)
    
    @staticmethod
    def from_message(meas_msg):
        return Measurement(meas_msg.x, meas_msg.y, meas_msg.id)
...
def data_association(predictions, covariances, measurements):
    ...
    for i in range(n_fsms):
        ...
        for j in range(n_measurements):
            if predictions[i, 2] == measurements[j, 2] and predictions[i, 2] != -1:
                diff = np.zeros((1, 2))
            else:
